...
{
  printk (KERN_ALERT "processing packet, data[0]: %d\n", data[0]);
...
rg_irq_in (struct urb *urb)
{
  ...
  printk (KERN_ALERT "urb callback with status: %d\n", urb->status);
...
static int
rg_start_input (struct rg_usb *rg)
{
  if (usb_submit_urb (rg->irq, GFP_KERNEL))
    return -EIO;
...
static void
rg_stop_input (struct rg_usb *rg)
{
  usb_kill_urb (rg->irq);
}

static int
rg_open (struct input_dev *dev)
{
  struct rg_usb *rg = input_get_drvdata (dev);

  return rg_start_input (rg);
}

static void
rg_close (struct input_dev *dev)
{
  struct rg_usb *rg = input_get_drvdata (dev);
...
static int
rg_init_input (struct rg_usb *rg)
{
  struct input_dev *input_dev;
  int i;

  printk (KERN_ALERT "initializing input\n");
  ...
  rg->dev = input_dev;
  input_dev->name = rg->name;
  input_dev->phys = rg->phys;
  usb_to_input_id (rg->udev, &input_dev->id);

  /* FIXME: remove this */
  printk (KERN_ALERT "usb name = %s\n", input_dev->name);

  input_dev->dev.parent = &rg->intf->dev;

  input_set_drvdata (input_dev, rg);

  input_dev->open = rg_open;
  input_dev->close = rg_close;

  /* set up standard buttons */
  for (i = 0; rg_common_btn[i] >= 0; i++)
    input_set_capability (input_dev, EV_KEY, rg_common_btn[i]);

  return 0;
}

static void
rg_deinit_input (struct rg_usb *rg)
{
  if (rg->input_created)
    {
      rg->input_created = false;
      input_unregister_device (rg->dev);
    }
}
...
rg_probe (struct usb_interface *intf, const struct usb_device_id *id)
{
  ...
  error = rg_init_input (rg);
  if (error)
    printk (KERN_ALERT "rg_init_input failed: %d\n", error);
  return 0;
